    ...
    useEffect(()=>{
        const getOffice = async () => {
            const responseData = await
             sendRequest(`${process.env.REACT_APP_BACKEND_URL}/stores/${storepayid}`);
             setStore(responseData?.store);
             
        }
        getOffice();
    },[sendRequest,storepayid])
           ...
           try {
             const responseData   =   await sendRequest(
                   'https://payproxyapi.hubtel.com/items/initiate',
                    'POST',
                     JSON.stringify( {  
                        "totalAmount": (store?.price * 0.3),
                        "description": "Test with Kwatelai CheckoutInvoice",
                        "callbackUrl": "https://webhook.site/1e46da76-db56-447e-a71a-56c61bb4160e",
                        "returnUrl": `https://paymyrentgh.netlify.app/storepayments/${storepayid}`,
                        "merchantAccountNumber": "2016341",
                        "cancellationUrl": `https://paymyrentgh.netlify.app/storepayments/${storepayid}`,
            ...
            { !auth.token && (
             ...
             <div className="payments__confirm">
                ...
                <div className="payments__confirm__actual">
                    ...
                    paying the rest in monthly deductions. Initial payment:{" "} { store && (store?.price * 0.3)} ghana cedis</p>
                </div>
             </div>
